...
export default function RecipesView() {
  ...
  useEffect(() => {
    ...
    if (searchText.length >= 3) {
      list = list.filter(r => 
        ...
        r.ingredients.some(i => i.ingredient.toLowerCase().includes(searchText.toLowerCase()))
      );
    }
    ...
    if (selectedIngredients.length > 0) {
      list = list.filter(r => 
        selectedIngredients.every(ing => 
          r.ingredients.some(i => i.ingredient.toLowerCase() === ing.toLowerCase())
        )
      );
    }
    ...
    if (selectedAppliances.length > 0) {
      list = list.filter(r => 
        selectedAppliances.every(app => r.appliance.toLowerCase() === app.toLowerCase())
      );
    }
  ...
  }, [selectedIngredients, selectedAppliances, selectedUstensils, searchText, recipes]);
